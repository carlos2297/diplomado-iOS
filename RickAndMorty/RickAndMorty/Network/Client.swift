...
struct Client {
    ...
    func get(_ path: String,query: [String:String] = [:], success: requesHandler?, failure: errorHandler? = nil) {
        request(method: "GET", path: path, body: nil,query: query, success: success, failure: failure)
    }
    
    func request(method: String, path: String, body: Data?,query: [String:String] = [:], success: requesHandler?, failure: errorHandler? = nil) {
        guard let request = buildRequest(method: method, path: path, body: body, query: query) else {
    ...
    private func buildRequest(method: String, path: String, body: Data?, query: [String:String] = [:]) -> URLRequest? {
